...
function getSearchString(input_title) {
	...
	if(input_title) {
		...
		//Create final query
		var finalQuery = "http://en.wikipedia.org" + start + title + properties + revisions + lists + list_parameters + "&callback=?";
	    return finalQuery;
	}
}
...
function getRandomSearchString() {
	...
	//Create final query
	var finalRandomQuery = "http://en.wikipedia.org" + start + random + "&callback=?";
...
function getMonth(word) {
	var result = -1;
	result = word.localeCompare("january");
	if(result == 0)
		return 1; 
	result = word.localeCompare("february");
	if(result == 0)
		return 2; 
	result = word.localeCompare("march");
	if(result == 0)
		return 3; 
	result = word.localeCompare("april");
	if(result == 0)
		return 4; 
	result = word.localeCompare("may");
	if(result == 0)
		return 5; 
	result = word.localeCompare("june");
	if(result == 0)
		return 6; 
	result = word.localeCompare("july");
	if(result == 0)
		return 7; 
	result = word.localeCompare("august");
	if(result == 0)
		return 8; 
	result = word.localeCompare("september");
	if(result == 0)
		return 9; 
	result = word.localeCompare("october");
	if(result == 0)
		return 10; 
